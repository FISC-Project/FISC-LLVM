...
const char *FISCTargetLowering::getTargetNodeName(unsigned Opcode) const {
  ...
  case FISCISD::MOVEi32:  return "MOVEi32";
	...
	: TargetLowering(FISCTM), Subtarget(*FISCTM.getSubtargetImpl()) 
{
  ...
  addRegisterClass(MVT::i32, &FISC::GRRegsRegClass);
...
SDValue FISCTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI, SmallVectorImpl<SDValue> &InVals) const {
  ...
  Chain =
	  DAG.getCALLSEQ_START(Chain, DAG.getIntPtrConstant(NumBytes, Loc, true), Loc);
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
	...
	SDValue StackPtr = DAG.getRegister(FISC::SP, MVT::i32);
...
bool FISCTargetLowering::CanLowerReturn(
...
SDValue
FISCTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
							   bool isVarArg,
